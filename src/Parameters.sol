...
contract Parameters is OwnableUpgradeable, UUPSUpgradeable {
    ...
    function _authorizeUpgrade(address implementationContract) internal override onlyOwner {}

    function setSkipSignatureValidation(bool signatureValidationFlag) public onlyOwner {
        SKIP_SIGNATURE_VALIDATION = signatureValidationFlag;
    }

    function setChallengeTimeoutPeriod(uint48 timeoutPeriod) public onlyOwner {
        CHALLENGE_TIMEOUT_PERIOD = timeoutPeriod;
    }

    function setDisputeSecurityDeposit(uint256 securityAmount) public onlyOwner {
        DISPUTE_SECURITY_DEPOSIT = securityAmount;
    }

    function setOperatorCollateralMinimum(uint256 collateralAmount) public onlyOwner {
        OPERATOR_COLLATERAL_MINIMUM = collateralAmount;
    }

    function setFinalizationDelaySlots(uint256 delaySlots) public onlyOwner {
